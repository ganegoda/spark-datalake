...
def process_song_data(spark, input_data, output_data):
    ...
    songs_table = song_df.select('song_id','title', 'artist_id', song_df.year.cast(IntegerType()), 'duration').dropDuplicates(['song_id']) 
    
    # write songs table to parquet files partitioned by year and artist
    songs_table.repartition("year","artist_id").write.mode('overwrite').partitionBy("year","artist_id").parquet(os.path.join(output_data, "songs.parquet"))
...
def process_log_data(spark, input_data, output_data):
    ...
    log_df = log_df.withColumn("time_stamp", to_utc_timestamp(from_unixtime(col("ts")/1000,'yyyy-MM-dd HH:mm:ss'),'PST'))
    ...
    users_table = log_df_song_play.select('user_id', 'first_name', 'last_name', 'gender','level').dropDuplicates(['user_id'])
    ...
    time_table.write.mode('overwrite').parquet(os.path.join(output_data, "times.parquet"))
    ...
    songplays_table = songplays_table.withColumn("year", year(col("time_stamp"))).withColumn("month", month(col("time_stamp")))

    # write songplays table to parquet files partitioned by year and month
    songplays_table.repartition("year", "month").write.mode('overwrite').partitionBy("year","month").parquet(os.path.join(output_data, "songplays.parquet"))
